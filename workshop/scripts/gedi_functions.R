...
# t.test
t.test(deforested_pts$rh100, forest_pts$rh100)


```

We can also made this comparisson by using the raster we generated before
```{r}

rh_max_forest <- mask(crop(rh100metrics[['mean']], forest_poly), forest_poly) 
plot(rh_max_forest)

rh_max_deforested <- mask(crop(rh100metrics[['mean']], deforested_poly), deforested_poly) 
plot(rh_max_deforested)

rh100Rast <- rbind.data.frame(cbind(rh100 = rh_max_deforested@data@values, label = 'Deforested', clr = "#F8766D"),
                              cbind(rh100 = rh_max_forest@data@values, label = 'Forest', clr = '#B79F00'))
rh100Rast$rh100 <- as.numeric(rh100Rast$rh100)

ggplot(data = rh100Rast, 
       aes(x = label, y = rh100, color = label, )) +
  geom_boxplot(fill=0.8, outlier.size=0.5, color = 'black') +  
  geom_point(position=position_jitter(width=0.3), alpha=0.8) +
  guides(colour = guide_legend(override.aes = list(alpha=1))) + 
...
# t.test
t.test(rh_max_deforested@data@values, rh_max_forest@data@values)
